...
export const createReservationService = async (user, data) => {
  ...
  try {
    ...
    // OBTENER URL DE PAGO
    const paymentUrl =
      transactionDetail?.payment_method?.extra?.async_payment_url;
...
export const getReservationByIdService = async (user, id) => {
  ...
  if (user.idRol === 3 && reserva.userId !== user.id) {
    throw new Error("No tienes permisos para ver esta reserva");
  }

  if (user.idRol === 2 && reserva.cancha.mallId !== user.idMall) {
    throw new Error("No tienes permisos para ver esta reserva");
  }
...
export const getReservationsByCourtService = async (user, courtId) => {
  const court = await Court.findByPk(courtId);
  if (!court) throw new Error("Cancha no encontrada");

  if (user.idRol === 2 && court.mallId !== user.idMall) {
    throw new Error("No tienes permisos para acceder a esta cancha");
  }
...
export const updateReservationService = async (user, id, data) => {
  const { fechaReserva, horaReserva, cantidadHoras, courtId } = data;
    ...
    include: [
      {
        model: Court,
        as: "cancha",
        attributes: ["id", "valorHora", "horarioInicio", "horarioFin", "diasDisponibles", "mallId"]
      }
    ]
  });
  ...
  if (courtId && courtId !== reserva.courtId) {
    throw new Error("No se puede cambiar la cancha de la reserva");
  }

  const nuevaFechaReserva = fechaReserva || reserva.fechaReserva;
  const nuevaHoraReserva = horaReserva || reserva.horaReserva;
  const nuevaCantidadHoras = cantidadHoras || reserva.cantidadHoras;

  if (!/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(nuevaHoraReserva)) {
    throw new Error("Formato de hora inválido. Use HH:MM (24 horas)");
  }

  if (nuevaCantidadHoras < 1 || nuevaCantidadHoras > 8) {
    throw new Error("La cantidad de horas debe estar entre 1 y 8");
  }

  validarHorarioCancha(reserva.cancha, nuevaHoraReserva, nuevaCantidadHoras, nuevaFechaReserva);
  validarDiasOperacion(reserva.cancha, nuevaFechaReserva);
  ...
  const [horaReservaH, horaReservaM] = nuevaHoraReserva.split(':').map(Number);
  const minutosInicioReserva = horaReservaH * 60 + horaReservaM;
  const minutosFinReserva = minutosInicioReserva + nuevaCantidadHoras * 60;

  for (const otraReserva of reservasSolapadas) {
    const [horaExistenteH, horaExistenteM] = otraReserva.horaReserva.split(':').map(Number);
    const minutosInicioExistente = horaExistenteH * 60 + horaExistenteM;
    const minutosFinExistente = minutosInicioExistente + otraReserva.cantidadHoras * 60;
    ...
    if (
      ...
      throw new Error(`La cancha ya está reservada de ${otraReserva.horaReserva} a ${otraReserva.cantidadHoras} horas`);
    }
  }

  reserva.fechaReserva = nuevaFechaReserva;
  reserva.horaReserva = nuevaHoraReserva;
  reserva.cantidadHoras = nuevaCantidadHoras;
  reserva.valorTotal = reserva.cancha.valorHora * nuevaCantidadHoras;
      ...
      include: [
        {
          model: Court,
          as: "cancha",
          attributes: ["id", "nombreCancha", "mallId"]
        }
      ]
    })
  };
